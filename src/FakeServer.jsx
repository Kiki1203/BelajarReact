...
export default class FakeServer extends Component {
  ...
  onSaveData = () => {
    ...
    if (username && email && password) {
      Axios.patch('http://localhost:5000/users/' + this.state.selectedId, { username, email, password })
        .then((res) => {
          if (res.status === 200) {
